...
class CdkPipelineStack(core.Stack):
    ...
    def __init__(self, scope: core.Construct, construct_id: str, **kwargs) -> None:
        ...
        synth_action= pipelines.SimpleSynthAction.standard_npm_synth(
            ...
            cloud_assembly_artifact= cloud_asm_artifact,
            install_command= 'npm install -g aws-cdk@1.84.0  && python -m pip install -r "requirements.txt" && pip3 install bandit',
            #skipping checks for s3 artifact bucket false positives
            build_command= 'cdk synth && bridgecrew -f cdk.out/CdkPipelineStack.template.json --skip-check CKV_AWS_7,CKV_AWS_18,CKV_AWS_21,CKV_AWS_111 && bandit -r lambda/'
            )
        )
